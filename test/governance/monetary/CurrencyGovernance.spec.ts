...
import { TrustedNodes__factory, TrustedNodes, CurrencyGovernance__factory, CurrencyGovernance, StageTestCurrencyGovernance__factory, StageTestCurrencyGovernance, Policy } from '../../../typechain-types'
...
const CYCLE_LENGTH = PROPOSE_STAGE_LENGTH + COMMIT_STAGE_LENGTH + REVEAL_STAGE_LENGTH
// enum values
...
describe('CurrencyGovernance', () => {
    ...
    describe('time calculations', () => {
      ...
      beforeEach(async () => {
        StageTestCG = await (await smock.mock<StageTestCurrencyGovernance__factory>('StageTestCurrencyGovernance')).deploy()
      })
      ...
      async function checkStageModifiers(cycle: Number, stage: Number) {
        if(stage === PROPOSE_STAGE) {
          ...
          await expect(StageTestCG.inVotePhase(cycle)).to.be.revertedWith(ERRORS.CurrencyGovernance.WRONG_STAGE)
          await expect(StageTestCG.inRevealPhase(cycle)).to.be.revertedWith(ERRORS.CurrencyGovernance.WRONG_STAGE)
        } else if (stage === COMMIT_STAGE) {
          await expect(StageTestCG.inProposePhase(cycle)).to.be.revertedWith(ERRORS.CurrencyGovernance.WRONG_STAGE)
          ...
          throw 'checkModifiers: bad stage input'
        }
      }
      ...
      describe('cycle tests', () => {
        ...
        it('test forward incompleteness', async () => {
          await expect(StageTestCG.cycleCompleted(completedCycles)).to.be.revertedWith(ERRORS.CurrencyGovernance.CYCLE_INCOMPLETE)
          await expect(StageTestCG.cycleCompleted(completedCycles+1)).to.be.revertedWith(ERRORS.CurrencyGovernance.CYCLE_INCOMPLETE)
          await expect(StageTestCG.cycleCompleted(completedCycles+2)).to.be.revertedWith(ERRORS.CurrencyGovernance.CYCLE_INCOMPLETE)
          await expect(StageTestCG.cycleCompleted(completedCycles+10)).to.be.revertedWith(ERRORS.CurrencyGovernance.CYCLE_INCOMPLETE)
          await expect(StageTestCG.cycleCompleted(completedCycles+1000)).to.be.revertedWith(ERRORS.CurrencyGovernance.CYCLE_INCOMPLETE)
        })
        ...
        it('test past completeness', async () => {
          ...
          expect(await StageTestCG.cycleCompleted(completedCycles-10)).to.be.true
          expect(await StageTestCG.cycleCompleted(0)).to.be.true
        })

        describe('test stage modifiers in wrong cycle', () => {
          it('propose phase', async () => {
            await expect(StageTestCG.inProposePhase(completedCycles-1)).to.be.revertedWith(ERRORS.CurrencyGovernance.CYCLE_INACTIVE)
          ...
          it('commit phase', async () => {
            await time.increase(PROPOSE_STAGE_LENGTH)
            await expect(StageTestCG.inVotePhase(completedCycles-1)).to.be.revertedWith(ERRORS.CurrencyGovernance.CYCLE_INACTIVE)
          ...
          it('reveal phase', async () => {
            await time.increase(REVEAL_STAGE_START)
            await expect(StageTestCG.inRevealPhase(completedCycles-1)).to.be.revertedWith(ERRORS.CurrencyGovernance.CYCLE_INACTIVE)
