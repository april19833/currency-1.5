...
describe.only('Migration tests', () => {
  ...
  context('with the proposal contstructed', () => {
    beforeEach(async () => {
      const implementationUpdatingTarget = await deploy(alice, ImplementationUpdatingTarget__factory)

      const proposalParams = [
        fixtureAddresses.communityGovernance,
        fixtureAddresses.ecoXExchange,
        // fixtureAddresses.lockupNotifier,
      ...
      proposal = await deploy(alice, MigrationLinker__factory, proposalParams) as MigrationLinker
    ...
    it('proposal constructs correctly', async () => {
      ...
      expect(await proposal.ecoxProxyAddress()).to.eq(baseContracts.ecox.address)
      
      expect(fixtureAddresses.ecox).to.not.eq(baseContracts.ecox.address)
      
      expect(await proposal.newEcoxStakingImpl()).to.eq(fixtureAddresses.ecoXStaking)
      
      expect(await proposal.ecoXStakingProxyAddress()).to.eq(baseContracts.ecoXStaking.address)
      
      expect(fixtureAddresses.ecoXStaking).to.not.eq(baseContracts.ecoXStaking.address)
      
      expect(await proposal.communityGovernance()).to.eq(fixtureAddresses.communityGovernance)
      ...
      expect(await proposal.rebaseNotifier()).to.eq(fixtureAddresses.rebaseNotifier)

      expect(await proposal.monetaryPolicyAdapter()).to.eq(fixtureAddresses.adapter)

      expect(await proposal.currencyGovernance()).to.eq(fixtureAddresses.monetaryGovernance)
    ...
    context.only('with enacted proposal', () => {
      ...
      beforeEach(async () => {
        ...
        await policyProposals
          .connect(alice)
          .registerProposal(proposal.address)
        ...
        baseContracts.policy = (new Policy__factory(alice)).attach(policyProxy.address)
        ...
        baseContracts.ecoXStaking = (new ECOxStaking__factory(alice)).attach(ecoXStakingProxy.address)
      })
      ...
      it('changes proxy implementations', async () => {
        ...
        const newEcoXStakingImpl = await baseContracts.ecoXStaking.implementation()
