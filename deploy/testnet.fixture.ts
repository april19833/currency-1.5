...
export async function deployCommunity(
    ...
    const communityGovernance = await deploy(wallet, CommunityGovernance__factory, [base.policy.address, base.eco.address, base.ecoXStaking.address, pauser]) as CommunityGovernance
...
export async function deployMonetary(
    ...
    const lockupsContract = await deploy(wallet, Lockups__factory, [base.policy.address, PLACEHOLDER_ADDRESS, base.eco.address, LOCKUP_DEPOSIT_WINDOW]) as Lockups
    const lockupsNotifier = await deploy(wallet, Notifier__factory, [base.policy.address, lockupsContract.address, [], [], []]) as Notifier
    const rebaseContract = await deploy(wallet, Rebase__factory, [base.policy.address, PLACEHOLDER_ADDRESS, base.eco.address]) as Rebase
    const rebaseNotifier = await deploy(wallet, Notifier__factory, [base.policy.address, rebaseContract.address, [], [], []]) as Notifier
    const adapter = await deploy(wallet, MonetaryPolicyAdapter__factory, [base.policy.address, PLACEHOLDER_ADDRESS]) as MonetaryPolicyAdapter
    const governance = await deploy(wallet, CurrencyGovernance__factory, [base.policy.address, PLACEHOLDER_ADDRESS, adapter.address]) as CurrencyGovernance
    const trustedNodes = await deploy(wallet, TrustedNodes__factory, [base.policy.address, governance.address, base.ecox.address, TRUSTEE_TERM, VOTE_REWARD, trustees]) as TrustedNodes
...
export async function deployBase(
    ...
    const ecoXStaking = await deployProxy(wallet, ECOxStaking__factory, [policy.address, ecox.address]) as ECOxStaking
...
export async function testnetFixture(
    ...
    const base: BaseContracts = await deployBase(wallet, pauser, initialECOxSupply)
    const monetary: MonetaryGovernanceContracts = await deployMonetary(wallet, base, trustees)
    const community: CommunityGovernanceContracts = await deployCommunity(wallet, base, pauser)
